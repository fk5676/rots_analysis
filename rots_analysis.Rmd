...
ggplot(parameters_rots_results) +
  geom_point(
    aes(
      results,
      concentration,
...
Standard model selection using an AIC (all are linear models so only dependent on residuals and number of values used).

```{r echo = FALSE}
# fit parameter reporting
knitr::kable(aic_test %>%
               arrange(AIC) %>%
               select(-a,-b) %>%
             ...
             col.names = c("Schudproef / Kolomproef (or combined)",
                           ...
                           "AIC",
                           "functional form",
                           "Soil concentration ($\\mu$g/kg ds)"))
```


### Full functional responses
...
df_results <- bind_rows(df, df_full)

m <- matrix(
  rep(conc, nrow(parameters_rots)),
  nrow(parameters_rots),
  length(conc),
  byrow = TRUE)
colnames(m) <- paste0("int_",1:length(conc),"_conc")

tmp <- cbind(parameters_rots, m)
...
p <- ggplot() +
  ...
  ylim(0,600) +
  labs(
    y = expression("soil concentration ("* mu * "g/kg ds)"),
    x = expression("effluent ("* mu * "g/l)"),
    caption = "maximum soil concentrations of PFOS
              depending on the standards used"
  ) +
  theme(
